...
class PeerListener {
	...
	func setupBonjourListener() {
		do {
            // When hosting a game via Bonjour, use the passcode and advertise the _MController._tcp service.
            ...
            // Set the service to advertise.
            listener.service = NWListener.Service(name: name, type: "_MController._tcp")
    ...
    func applicationServiceListenerStateChanged(newState: NWListener.State) {
        ...
        case .waiting(let error):
            print("Listener waiting because \(error)")
        default:
            break
        }
    }
	...
	func resetName(_ name: String) {
        ...
        if let listener = listener {
            ...
            listener.service = NWListener.Service(name: self.name, type: "_MController._tcp")
        }
	}
}
