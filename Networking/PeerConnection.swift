...
func applicationServiceParameters() -> NWParameters {
    ...
    let controlOptions = NWProtocolFramer.Options(definition: ControlProtocol.definition)
    parameters.defaultProtocolStack.applicationProtocols.insert(controlOptions, at: 0)
...
class PeerConnection {
	...
	func receiveNextMessage() {
		...
		connection.receiveMessage { (content, context, isComplete, error) in
			...
			if let controlMessage = context?.protocolMetadata(definition: ControlProtocol.definition) as? NWProtocolFramer.Message {
				self.delegate?.receivedMessage(content: content, message: controlMessage)
			}
			if error == nil {
