...
class ControlProtocol: NWProtocolFramerImplementation {

	// Create a global definition of your game protocol to add to connections.
	static let definition = NWProtocolFramer.Definition(implementation: ControlProtocol.self)

	// Set a name for your protocol for use in debugging.
	static var label: String { return "MController" }
	...
	func handleOutput(framer: NWProtocolFramer.Instance, message: NWProtocolFramer.Message, messageLength: Int, isComplete: Bool) {
		...
		let type = message.controlMessageType

		// Create a header using the type and length.
		let header = ControlProtocolHeader(type: type.rawValue, length: UInt32(messageLength))
	...
	func handleInput(framer: NWProtocolFramer.Instance) -> Int {
		while true {
			...
			var tempHeader: ControlProtocolHeader? = nil
			let headerSize = ControlProtocolHeader.encodedSize
			let parsed = framer.parseInput(minimumIncompleteLength: headerSize,
				...
				tempHeader = ControlProtocolHeader(buffer)
				return headerSize
			}
			...
			var messageType = ControlMessageType.invalid
			if let parsedMessageType = ControlMessageType(rawValue: header.type) {
				messageType = parsedMessageType
			}
			let message = NWProtocolFramer.Message(controlMessageType: messageType)
...
extension NWProtocolFramer.Message {
	convenience init(controlMessageType: ControlMessageType) {
		self.init(definition: ControlProtocol.definition)
		self["ControlMessageType"] = controlMessageType
	}

	var controlMessageType: ControlMessageType {
		if let type = self["ControlMessageType"] as? ControlMessageType {
			return type
		} else {
			return .invalid
		}
...
struct ControlProtocolHeader: Codable {
	let type: UInt32
	let length: UInt32
